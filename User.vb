Public Class User
    ...
    Public Sub Simpan()
        ...
        If (user_baru = True) Then
            strsql = "Insert into user(iduser,username,nama_lengkap,passwd) values ('" & _iduser & "','" & _username & "','" & _nama_lengkap & "','" & _passwd & "')"
            info = "INSERT"
        Else
            strsql = "update user set iduser='" & _iduser & "', username='" & _username & "', nama_lengkap='" & _nama_lengkap & "', passwd='" & _passwd & "' where iduser='" & _iduser & "'"
            info = "UPDATE"
        End If
        ...
        DBDisconnect()
    End Sub
    Public Sub Cariuser(ByVal sidbarang As String)
        DBConnect()
        strsql = "SELECT * FROM user WHERE idbarang='" & sidbarang & "'"
        myCommand.Connection = conn
        myCommand.CommandText = strsql
    ...
    Public Sub Hapus(ByVal sidbarang As String)
        Dim info As String
        DBConnect()
        strsql = "DELETE FROM user WHERE idbarang='" & sidbarang & "'"
        info = "DELETE"
        myCommand.Connection = conn
        myCommand.CommandText = strsql
        Try
